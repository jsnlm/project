...
def RandomMode(mode, hScore):
    #-------------------Instantiate variables and other things.---------------------------
    #hScore = highscore()    #initiate Highscorse
    hScore.mainDecision=True
    hScore.mainDecision2=True
    pygame.init()
    ...
    congratulation = pygame.mixer.Sound("resources/Congratulation.wav")
...
#------------------Show the Instrcutions Page 1------------------------------
    while continu == False and hScore.mainDecision == True:
        #check for events  that happened in pygame
        for event in pygame.event.get(): # User did something
            if event.type == pygame.QUIT: # If user clicked close
                hScore.mainDecision = False#if user exited the window, it will make done false so that is by passes all the coming for loops.

        screen.blit(InstructionsPic1, [0, 0])   #show the instructions on the screen

        #prompt the user to click the screen to get to the next page
        InstructionText=InstructionsFont.render("click anywhere to Continue", True, red) # promp the user to click to contunue
        screen.blit(InstructionText, [(screen_width/2-170), 550])

        press = pygame.mouse.get_pressed()

        #if the mouse is clicked, then conitnue will be set to true so that it can go to the next part of the game
        if press[0] == 1:
            continu = True

        pygame.display.flip()

    continu = False #reset continue to True

    press = pygame.mouse.get_pressed()
#------------------This loop will run while the user is clicking the mouse.
#------------------This is to check that the user lets go of the mouse before going to the next page of instructions. ------------------------------
    while press[0] == 1:
        for event in pygame.event.get(): # User did something
            if event.type == pygame.QUIT: # If user clicked close
                hScore.mainDecision = False #if user exited the window, it will make done false so that is by passes all the coming for loops.
        ...
        press = pygame.mouse.get_pressed()
#------------------Show the Instrcutions Page 2------------------------------
    while continu == False and hScore.mainDecision == True:
        for event in pygame.event.get(): # User did something
            if event.type == pygame.QUIT: # If user clicked close
                hScore.mainDecision = False#if user exited the window, it will make done false so that is by passes all the coming for loops.

        screen.blit(InstructionsPic2, [0, 0])   #show the instructions on the screen
    ...
    while hScore.mainDecision == True and hScore.mainDecision2==True:
        hScore.mainDecision=True
        hScore.mainDecision2=True
        ...
        while gameover == True:
            ...
            if beatHighscore == True:
                ...
                gameOverText1=gameOverFont1.render("Congratulations", True, red)
                screen.blit(gameOverText1, [(screen_width/2-280), (screen_height/2 - 100)])

                gameOverText1=gameOverFont1.render("New Highscore", True, red)
                screen.blit(gameOverText1, [(screen_width/2-245), (screen_height/2 - 30)])
            else:
        ...
        while (hScore.mainDecision == True) and (pause == True) and (hScore.mainDecision2 == True):
            for event in pygame.event.get(): # User did something
                if event.type == pygame.QUIT: # If user clicked close
                    gameover = False
                    hScore.mainDecision = False#if user exited the window, it will make done false so that is by passes all the coming for loops.
            ...
            #----------------This will print text to the screen after the player loses----------------------
            gameOverText=gameOverFont1.render("Paused", True, red)
            screen.blit(gameOverText, [(screen_width/2-130), (screen_height/2 - 30)])

            gameOverText=gameOverFont2.render("Click this to continue", True, red)
            screen.blit(gameOverText, [(screen_width/2-160), (screen_height/2 + 60)])

            gameOverText=gameOverFont2.render("Click this to go back to menu", True, red)
            screen.blit(gameOverText, [(screen_width/2-160), (screen_height/2 + 120)])
            ...
            if press[0] == 1:
                ...
                if (pos[0]>=133.9) and (pos[0]<=397) and (pos[1]>=389)  and (pos[1]<=417):
                    pause = False
                    pygame.mouse.set_pos(player.getXPosition(), player.getYPosition())
                elif (pos[0]>=133.9) and (pos[0]<=476) and (pos[1]>=449)  and (pos[1]<=488):
                    hScore.mainDecision2 = False
                    print "hScore.mainDecision2 = False"
        ...
        #----------------------What happens when the player is playing the game.
        while (gameover == False) and (hScore.mainDecision == True) and (pause == False) and (hScore.mainDecision2 == True):
            for event in pygame.event.get(): # User did something
                if event.type == pygame.QUIT: # If user clicked close
                    hScore.mainDecision = False #if user exited the window, it will make done false so that is by passes all the coming for loops.
                    gameover = True
            ...
            if len(blackBallHitList) != 0:
                if currentItem == 0:
                    ...
                    if score>hScore.getScore(mode):     #This will save the player's score if it is higher than the curren highscore
                        ...
                        beatHighscore = True
                        VoiceSounds.play(congratulation)
                    else:
            ...
            #CHecks if the user clicker the screen. THis is to check if the user paused the game
            press = pygame.mouse.get_pressed()
            if press[0] == 1:
                pause = True
