...
class Player(Block):
    ...
    def __init__(self, color, width, height):
        Block.__init__(self, color, width, height)
        self.invincibleImage = pygame.image.load('Invincible.png')                  #inivincible image is loaded from file
...
class Star(Block):
    ...
    def update(self):
        ...
        self.rect.y=random.randrange(screen_height - 15)      #sets random value for y position
...
class Item(Block):
    ...
    def update(self):
        ...
        self.rect.y=random.randrange(screen_height - 15)
    #pre:
